...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    efficiency = ['green', 'orange', 'red', 'black']
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    operation = temperature * neutrons_produced_per_second
    
    if operation < (threshold * 0.9):
        status = 'LOW'
    elif operation >= (threshold - (threshold * 0.1)) and operation <= (threshold + (threshold * 0.1)):
        status = 'NORMAL'
    else:
        status = 'DANGER'
    return status
